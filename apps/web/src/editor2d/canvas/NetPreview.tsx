...
// 색은 크림색(표면색) 하나로 칠하고, 윗면·옆면에 약한 그림자를 준다.
// 요소 배치·핸들은 Phase 3에서 추가된다.
...
export function NetPreview() {
  ...
  return (
    ...
    >
      <defs>
        {/* 약한 그림자 */}
        <filter id="net-soft-shadow" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow
            dx={0}
            dy={0.8}
            stdDeviation={1}
      ...
      {/* 윗면 */}
      <text x={topX + topW / 2} y={topY - 2} textAnchor="middle" style={labelStyle}>
        윗면 · TOP
      </text>
      <path d={topPath} fill={creamColor} filter="url(#net-soft-shadow)" />

      {/* 옆면(전개) — 폭이 둘레에 비례 */}
      <text x={sideX + sideW / 2} y={sideY - 2} textAnchor="middle" style={labelStyle}>
        옆면 · SIDE (전개)
      </text>
      <rect
        x={sideX}
