...
// 조율한다. 영속화는 infra 저장소를 통해, 토큰은 share를 통해 한다.
...
export interface DesignService {
  ...
  create(input: unknown): CreateResult;
  /** 편집 토큰으로 로드(작성자용). 작성자에겐 두 링크를 함께 돌려준다. */
  getByEdit(editToken: string): CreateResult;
  /** 열람 토큰으로 로드(읽기). 편집 토큰은 노출하지 않는다. */
  getByView(viewToken: string): Design;
  /** 편집 토큰으로 작성자 수정 저장. id는 원본 유지. */
  updateByEdit(editToken: string, input: unknown): Design;
  /** 열람 토큰으로 복제 — 새 id·새 토큰으로 독립 디자인 생성. */
  cloneByView(viewToken: string): CreateResult;
}

/** 입력을 서버 id로 강제한 뒤 검증해 저장하고 토큰을 발급하는 공통 경로. */
function persistNew(repo: DesignRepository, input: unknown): CreateResult {
  const id = randomUUID();
  ...
  repo.linkToken(shareLink.editToken, id, 'edit');
  repo.linkToken(shareLink.viewToken, id, 'view');
  repo.saveShareLink(shareLink);
  return { design, shareLink };
}

export function createDesignService(repo: DesignRepository): DesignService {
  function requireDesign(token: string, role: 'edit' | 'view'): Design {
    const id = repo.resolveToken(token, role);
    const design = id ? repo.getDesign(id) : undefined;
  ...
  return {
    ...
    getByEdit(editToken) {
      const design = requireDesign(editToken, 'edit');
      const shareLink = repo.getShareLink(design.id);
    ...
    updateByEdit(editToken, input) {
      const existing = requireDesign(editToken, 'edit');
    ...
    cloneByView(viewToken) {
      const source = requireDesign(viewToken, 'view');
